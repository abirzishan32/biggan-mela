...
export default function GraphControls({
  ...
  // Fixed handleGenerateGraph function to properly create graphs
  const handleGenerateGraph = () => {
    try {
      ...
      if (graphType === 'teaching') {
        ...
        // Set default starting node for teaching graph
        onGraphChange(newGraph);
        onStartNodeChange('A');
      } else {
        newGraph = generateRandomGraph(nodeCount, edgeProbability);
        onGraphChange(newGraph);
        // Set first node as default starting node for random graph
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3">
      ...
      {/* First row: Graph configuration with two columns */}
      <div className="grid grid-cols-2 gap-2 mb-2">
        ...
        <div>
          <label className="block text-xs font-medium mb-1">Starting Node</label>
          <select
      ...
      {/* Second row: Random graph parameters (conditionally rendered) */}
      {graphType === 'random' && (
        <div className="grid grid-cols-2 gap-2 mb-2">
          <div>
      ...
      {/* Third row: Generate and Reset buttons */}
      <div className="grid grid-cols-2 gap-2 mb-3">
        <button
          onClick={handleGenerateGraph}
          className="px-2 py-1 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50"
          disabled={isRunning}
        >
          Generate Graph
        </button>
