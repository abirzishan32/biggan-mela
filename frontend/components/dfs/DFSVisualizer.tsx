...
export default function DFSVisualizer() {
  ...
  // Generate DFS steps when graph or start node changes
  useEffect(() => {
    if (graph && startNode) {
      try {
  ...
  // Current step data
  const currentStep: DFSStep | null = 
  ...
  return (
    <div className="container mx-auto px-2 py-4">
      <h1 className="text-2xl font-bold text-center mb-4">DFS Algorithm Visualization</h1>
      
      {/* Compact layout grid */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
        {/* Main visualization area - 8 columns */}
        <div className="lg:col-span-8 flex flex-col gap-3">
          {/* Top row with controls and stack side by side */}
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
            ...
            {/* Stack - 4 columns, next to controls */}
