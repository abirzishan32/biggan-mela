...
export default function QuizPage() {
    ...
    // Add exam status to track state clearly
    ...
    // Tab visibility change detection - FIXED IMPLEMENTATION
    useEffect(() => {
        // Only monitor tab visibility when quiz is in active progress
        if (examStatus !== "in-progress") return;

        console.log("Setting up visibility change detection");
        ...
        // Handler for blur event as a backup detector
        const handleBlur = () => {
            console.log("Window blur detected");
            if (examStatus === "in-progress") {
                console.log("Window lost focus - triggering disqualification");
                handleCheatingDetected("Window lost focus during exam");
            }
        };
        ...
        // Cleanup - FIXED! Now properly removes the blur event listener
        return () => {
            document.removeEventListener("visibilitychange", handleVisibilityChange);
            window.removeEventListener("blur", handleBlur); // FIXED: Changed addEventListener to removeEventListener
            console.log("Visibility change detection removed");
        };
    }, [examStatus]);
    ...
    const handleCheatingDetected = (reason?: string) => {
        ...
        // Set disqualification state immediately
        setDisqualificationReason(disqualificationMessage);
        setIsDisqualified(true);
        setExamStatus('disqualified');

        // Submit the quiz with current answers - using setTimeout to ensure state updates
        // before submission processing begins
        setTimeout(() => {
            submitQuiz();
        }, 0);
    };
    ...
    // Show camera setup screen if not checked yet
    // if (!quizStarted && !cameraPermissionChecked && !isLoading) {
    //     return (
    //         <motion.div
    //             initial={{ opacity: 0, y: 20 }}
    //             animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="flex gap-2 min-w-max">
                                {quiz.questions.map((q: any, index: number) => (
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        className={`h-9 w-9 p-0 rounded-md ${index === currentQuestionIndex
